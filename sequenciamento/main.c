    ...
    for (int i = 0; i < numero_doencas; i++) {
        ...
        for (int g = 0; g < numero_genes; g++) {
            ...
            if (total_subcadeias > 0) {
                int* coberto = (int*)calloc(len_gene, sizeof(int));
                ...
                for (int c = 0; c < len_gene; c++) {
                    total_cobertos += coberto[c];
                }
                free(coberto);

                double compatibilidade = ((double)total_cobertos / len_gene) * 100.0;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < numero_doencas; i++) {
        fprintf(outfile, "%s->%ld%%\n", lista_doencas[i].nome, lround(lista_doencas[i].probabilidade));
    }
